...
func pollGitHubReview(s *State, branch string) (*reviewSnapshot, error) {
	...
	for _, r := range d.Reviews {
		if eligibleFeedback(r.Author.Login, r.Body) && r.State == "CHANGES_REQUESTED" {
			snapshot.Feedback = append(snapshot.Feedback, feedbackInput{ID: stableID("github-review", r.ID), Source: "github-review", RemoteID: r.ID, Body: r.Body, Author: r.Author.Login, Group: "github-pr", RequestedChanges: true, HeadOID: pr.HeadOID})
		}
	}
...
func githubReviewFeedback(s *State, pr *Pull) ([]feedbackInput, []feedbackInput, error) {
	...
	for _, comment := range comments.Nodes {
		...
		flat = append(flat, feedbackInput{ID: stableID("github-comment", comment.ID), Source: "github-comment", RemoteID: comment.ID, Body: comment.Body, Author: comment.Author.Login, URL: comment.URL, Group: "github-pr", PositiveReactors: reactors, HeadOID: pr.HeadOID})
	}
	for _, thread := range threads.Nodes {
		...
		for _, comment := range thread.Comments.Nodes {
			if eligibleFeedback(comment.Author.Login, comment.Body) {
				...
				inline = append(inline, feedbackInput{ID: stableID("github-thread", thread.ID, comment.ID), Source: "github-thread", RemoteID: comment.ID, ThreadID: thread.ID, Body: comment.Body, Author: comment.Author.Login, URL: comment.URL, Path: thread.Path, Group: "github-thread:" + thread.ID, PositiveReactors: reactors, Line: line, Inline: true, Threaded: true, HeadOID: pr.HeadOID})
			}
		}
	}
...
func pollGitLabReview(s *State, branch string) (*reviewSnapshot, error) {
	...
	for _, discussion := range discussions {
		for _, note := range discussion.Notes {
			...
			input := feedbackInput{ID: stableID("gitlab-discussion", discussion.ID, strconv.Itoa(note.ID)), Source: "gitlab-discussion", RemoteID: strconv.Itoa(note.ID), ThreadID: discussion.ID, Body: note.Body, Author: note.Author.Username, Group: "gitlab-discussion:" + discussion.ID, HeadOID: mr.HeadOID, Inline: !discussion.Individual || note.Position != nil, Threaded: !discussion.Individual}
