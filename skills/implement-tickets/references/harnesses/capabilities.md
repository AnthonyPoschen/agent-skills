...
| Capability | Capable interactive harness (Codex or Grok) | Headless harness |
| --- | --- | --- |
| Worker fan-out and sandboxing | Prefer native agents and sandbox controls. Grok uses background `spawn_subagent` with a prepared checkout | Use supervisor launch and policy adapters |
...
| User-visible progress | Keep an active relay turn; use commentary headed by `Events`. Grok may also wake the next turn from a monitor line or scheduled poll | Stream `events --follow` to the host UI |
...
supervisor can keep implementing, but it cannot inject output into a Codex or
Grok turn that has already ended. Codex therefore keeps a bounded wait active
until the run reaches a terminal state and replays durable sequence-numbered
events after any reconnect. Grok keeps that same in-turn reap loop, then also
starts a review monitor and a scheduled poll so the next manager turn can
continue after the current answer. Only events confirmed as shown advance the
chat relay cursor.
