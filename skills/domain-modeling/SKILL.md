...
description: >
  Build and sharpen a project's domain model. Use when pinning down domain
  terminology or ubiquitous language, recording an architectural decision, or
  when another skill (for example grill-with-docs) needs to maintain the domain
  model. Also use when the repo declares short living alignment docs under
  docs/agents/ that should stay current as terms and constraints crystallise.
---
...
## Living short alignment docs (optional)

Some repos declare short living alignment files (environment map, landmines,
definition of done / evidence, golden-path smoke, pins, owns/asks) so agents do
not rediscover the same facts every session.

1. Read `docs/agents/domain.md` if it exists (or another repo-local domain index).
2. If that file names paths for short living docs, create or update those files
   when the relevant fact crystallises — same discipline as CONTEXT/ADR: short
   bullets, one screen when possible, no essay piles.
3. If the repo does not declare those paths, do **not** invent a new docs tree.
   Stick to `CONTEXT.md` and `docs/adr/`.

Examples of living-short content (only when the repo asks for them):

environment map, never-do landmines, work-type evidence / DoD, golden-path smoke
checklists, current pins, owns/asks.

## Source

First-party adaptation of the domain-modeling skill previously used via
mattpocock/skills, owned here so it can evolve with AnthonyPoschen/agent-skills.
